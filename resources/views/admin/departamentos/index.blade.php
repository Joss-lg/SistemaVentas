...
@section('content')
<div x-data="{ modalCrear: false, modalEditar: null }" class="p-6 max-w-7xl mx-auto space-y-6">

    {{-- ENCABEZADO --}}
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black italic uppercase tracking-wider text-zinc-900 dark:text-white">
                Gestión de <span class="text-red-600">Departamentos</span>
            </h1>
            <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Categoriza tus productos para un mejor control de inventario.</p>
        </div>
        <button @click="modalCrear = true" 
            class="px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold italic uppercase text-xs tracking-wider rounded-xl shadow-lg shadow-red-600/30 transition-all flex items-center justify-center space-x-2 cursor-pointer">
    ...
    {{-- ALERTAS DE SESIÓN --}}
    @if(session('success'))
        <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-bold flex items-center space-x-2">
    ...
    @if(session('error'))
        <div class="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-bold flex items-center space-x-2">
    ...
    <div class="bg-white dark:bg-[#0d0d0d] border border-zinc-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-white/5 text-xs font-bold text-zinc-800 dark:text-zinc-200">
                    @forelse($departamentos as $dep)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="p-4 font-mono text-zinc-400">#{{ $dep->id }}</td>
                            <td class="p-4 uppercase italic font-black">{{ $dep->nombre }}</td>
                            <td class="p-4 text-zinc-500 font-normal">{{ $dep->descripcion ?? 'Sin descripción' }}</td>
                            <td class="p-4">
                                <span class="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider {{ $dep->activo ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-zinc-500/10 text-zinc-400 border border-zinc-500/20' }}">
                                    {{ $dep->activo ? 'Activo' : 'Inactivo' }}
                                </span>
                            </td>
                            <td class="p-4 text-right space-x-2 whitespace-nowrap">
                                {{-- BOTÓN EDITAR --}}
                                <button @click="modalEditar = {{ $dep->id }}" class="px-3 py-1.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 text-xs font-bold transition-colors">
                                    <i class="fas fa-edit mr-1"></i> Editar
                                </button>

                                {{-- BOTÓN ELIMINAR --}}
                                <form action="{{ route('departamentos.destroy', $dep->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Seguro que deseas eliminar este departamento?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-3 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-500 text-xs font-bold transition-colors">
                                ...
                                <form action="{{ route('departamentos.update', $dep->id) }}" method="POST">
                                    ...
                                    <div class="p-4 border-b border-zinc-100 dark:border-white/5 flex justify-between items-center">
                                        <h3 class="text-sm font-black italic uppercase tracking-wider text-zinc-900 dark:text-white">
                                            Editar <span class="text-red-600">Departamento</span>
                                        </h3>
                                        <button type="button" @click="modalEditar = null" class="text-zinc-400 hover:text-white text-base">&times;</button>
                                    </div>
                                    ...
                                    <div class="p-5 space-y-4 text-left">
                                        ...
                                        <div class="flex items-center space-x-3 pt-2">
                                            <input type="checkbox" name="activo" value="1" id="activo{{ $dep->id }}" {{ $dep->activo ? 'checked' : '' }}
                                                class="w-4 h-4 rounded border-zinc-300 dark:border-white/10 text-red-600 focus:ring-red-600 bg-zinc-100 dark:bg-white/5">
                                    ...
                                    <div class="p-4 border-t border-zinc-100 dark:border-white/5 flex justify-end space-x-3 bg-zinc-50 dark:bg-white/[0.01]">
                                        <button type="button" @click="modalEditar = null" class="px-4 py-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-zinc-700 dark:hover:text-white transition-colors">
                                            Cancelar
                                        </button>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-8 text-center text-zinc-500 dark:text-zinc-400 italic">
    ...
    <div x-show="modalCrear" 
        ...
        <div @click.away="modalCrear = false" class="bg-white dark:bg-[#121212] border border-zinc-200 dark:border-white/10 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <form action="{{ route('departamentos.store') }}" method="POST">
                ...
                <div class="p-4 border-b border-zinc-100 dark:border-white/5 flex justify-between items-center">
                    <h3 class="text-sm font-black italic uppercase tracking-wider text-zinc-900 dark:text-white">
                        Nuevo <span class="text-red-600">Departamento</span>
                    </h3>
                    <button type="button" @click="modalCrear = false" class="text-zinc-400 hover:text-white text-base">&times;</button>
                </div>
                ...
                <div class="p-4 border-t border-zinc-100 dark:border-white/5 flex justify-end space-x-3 bg-zinc-50 dark:bg-white/[0.01]">
                    <button type="button" @click="modalCrear = false" class="px-4 py-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-zinc-700 dark:hover:text-white transition-colors">
                        Cancelar
                    </button>
                    <button type="submit" class="px-5 py-2 bg-red-600 hover:bg-red-700 text-white font-bold italic uppercase text-xs tracking-wider rounded-xl transition-all shadow-md shadow-red-600/20 cursor-pointer">
                        Crear
